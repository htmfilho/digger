...
A few moments later, open your browser and visit the address http://localhost:8080 to use Digger with its embedded database. The folder `data` is automatically created during the initialization.
...
1. stop the application using the `[Ctrl+C]` shortcut
...
5. Then open the file `application-server.properties` and adapt the following connection parameters to your PostgreSQL server:
