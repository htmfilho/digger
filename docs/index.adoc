...
1. Update and review the documentation to make sure it covers all the recent changes in the tool.

2. Increment the version number in the documentation to the version that is about to be released.

3. Generate the HTML version of the documentation:

    $ asciidoctor docs/index.adoc

4. Commit all the changes in the documentation:

    $ git add [list-of-modified-files]
    $ git commit -m "Updated the documentation for the release 1.3.0"

5. Check if there is any missing file to be committed in the project. If not, the master branch is ready to be tagged.

6. Push all local changes to the release branch:

    $ git push origin 1.4.0

7. Create a pull request to merge the release branch with the master branch, review the code to be merged and merge it.

8. Create the next milestone.

9. Move the unfinished work in the current milestone to the next milestone.

10. Close the current milestone.

11. Write the release notes.

12. Generate the package:

    $ mvn clean package

13. Upload the package to the release page.

14. Publish the release.

15. Create a branch for the next release:

    $ git checkout -b 1.5.0

16. Increment the version number in the pom file and commit it:

    $ git add pom.xml
    $ git commit -m "Incremented release number to 1.5.0"

17. Push the new branch to `origin`:

    $ git push origin 1.5.0

18. Announce the new release to the community and on social networks.
