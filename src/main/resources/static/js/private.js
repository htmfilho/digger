...
$(function() {
    if ($("#users").length != 0) {
        $.ajax({
            ...
            success: function(result) {
                result.forEach(element => {
                    $("#users").append('<tr><td><a href="/admin/users/'+ element.id +'">'+ element.username +'</a></td><td><input type="checkbox" id="enabled-'+ element.id +'" name="enabled" value="'+ element.username +'" '+ (element.enabled ? 'checked': '') +' onclick="enableUser(\''+ element.username +'\', this);"></td></tr>');
                });
            }
        });
    }
